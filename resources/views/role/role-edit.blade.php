...
@section('subhead')
<title>Form Update Role User - Midone - Tailwind HTML Admin Template</title>
@endsection

@section('subcontent')
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">Form Update Role User</h2>
</div>
        ...
        <form action="{{ route('roles.update', $role) }}" method="POST">
            ...
            <div class="intro-y box p-5">
                <div>
                    ...
                    <input id="crud-form-1" name="name" id="name" value="{{ $role->name ?? old('name') }}" type="text" class="form-control w-full" placeholder="Input Role Name">
                </div>
                {{-- <div class="mt-3">
                ...
                <div class="mt-3">
                    ...
                    <select data-placeholder="Pilih satu atau lebih Permission" name="permissions[]" class="tom-select w-full" id="post-form-3" multiple>
                        <option value="select-all">Select All</option>
                        @foreach ($permissions as $id => $name)
                            <option value="{{ $id }}" @if(in_array($id, old('permissions', [])) || $role->permissions->contains($id)) selected @endif>{{ $name }}</option>
                        @endforeach
                    </select>
                    @error('permissions')
